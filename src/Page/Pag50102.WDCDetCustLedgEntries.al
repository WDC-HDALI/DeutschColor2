page 50102 "WDC Det Cust. Ledg. Entries"
{
    AdditionalSearchTerms = 'sales transaction,payment receipt';
    ApplicationArea = Basic, Suite;
    Caption = 'WDC Det Cust. Ledg. Entries';
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Update Entry")
            {
                ...
                trigger OnAction()
                var
                    lcust: Record "Detailed Cust. Ledg. Entry";
                begin
                    lcust.Reset();
                    lcust.SetFilter("Entry No.", '%1|%2', 603, 602);
                    If lcust.FindFirst() Then
                        if lcust.Count = 2 then
                            lcust.ModifyAll("Transaction No.", 0);
                    lcust.ModifyAll("Application No.", 602);

                    lcust.Reset();
                    lcust.SetFilter("Entry No.", '%1|%2', 601, 600);
                    If lcust.FindFirst() Then
                        if lcust.Count = 2 then
                            lcust.ModifyAll("Transaction No.", 743);
                    lcust.ModifyAll("Application No.", 0);
                end;
            }
        }
    }
